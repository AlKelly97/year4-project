...
async function addBlog() {
    try {
        ...
        const makeBlog = { ...formState }
        setBlogs([...Blog, makeBlog])
        setFormState(initialState)
        console.log(formState)
        await API.graphql(graphqlOperation(createBlog, {input: makeBlog}))
    } catch (err) {
...
async function removeBlog(event) {
    ...
    console.log(val)
    const DeleteBlog = await API.graphql(graphqlOperation(deleteBlog, {input: {id: val}}));
    ...
    return (
        ...
        Blogs.map((Blog, index) => (
          <div> <li key={Blog.id ? Blog.id : index}>
            {Blog.name}</li>
            <a href={`Content/${Blog.id}`}>click me
                </a>
